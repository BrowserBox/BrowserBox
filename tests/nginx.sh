...
USAGE
  ...
  if [[ "$SKIP_PRECLEAN" != "true" ]]; then
    ...
    cleanup_user_nginx
    # We just need to clear the /etc/hosts entries from the previous run.
    mapfile -t hosts < <(parse_hosts_from_config)
    if ((${#hosts[@]})); then
      remove_hosts_entries "${hosts[@]}"
    fi
...
GUIDE
  ...
  # Define ports and hosts as arrays for easier iteration in the nginx config
  local ports=("$p0" "$p1" "$p2" "$p3" "$p4")
  local hosts=("$h0" "$h1" "$h2" "$h3" "$h4")

  # Optional: /etc/hosts in local mode
...
EOF
  ...
  # Loop to generate each server block and append it to the config
  for i in "${!ports[@]}"; do
    NCONF+=$(cat <<EOF
