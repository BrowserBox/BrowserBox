  ...
  if [[ -f "${ssl_dir}/privkey.pem" && -f "${ssl_dir}/fullchain.pem" && "$HOSTNAME" == "$output" ]]; then
    ...
    if [[ "$HOSTNAME" != "localhost" ]]; then
      ...
      if [[ -f ./wait_for_hostname.sh ]]; then
        ...
        bash <(curl -s https://raw.githubusercontent.com/BrowserBox/BrowserBox/main/deploy-scripts/wait_for_hostname.sh) $HOSTNAME
      fi
    fi
    ...
    if [[ -f ./tls ]]; then
      ...
      bash <(curl -s https://raw.githubusercontent.com/BrowserBox/BrowserBox/main/deploy-scripts/tls) $HOSTNAME
    fi
  fi
}
