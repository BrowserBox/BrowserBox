...
# Keep Windows behavior consistent: --full-install should also make `browserbox` available on PATH.
$copyScript = Join-Path $PSScriptRoot "cp_commands_only.ps1"
if (Test-Path $copyScript) {
    Write-Host "Copying BrowserBox command wrappers and binary to your PATH..." -ForegroundColor Cyan
    & $copyScript | Out-Null
    Write-Host "'browserbox' and 'bbx' should now be available on PATH (open a new terminal if needed)." -ForegroundColor Green
} else {
    Write-Warning "cp_commands_only.ps1 not found; run 'browserbox --install' to copy the binary to your PATH."
}
