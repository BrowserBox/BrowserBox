...
# Helper Functions
function Is-IpLiteral {
    param ([string]$Hostname)
    if (-not $Hostname) { return $false }
    $parsedIp = $null
    return [System.Net.IPAddress]::TryParse($Hostname, [ref]$parsedIp)
}

function Is-LocalHostname {
    param ([string]$Hostname)
    if (-not $Hostname) { return $true }
    if ($Hostname -like "*.onion") { return $true }
    if (Is-IpLiteral $Hostname) { return $true }
...
# Ensure mkcert is available (install via winget if missing).
function Ensure-Mkcert {
    $mkcertCmd = Get-Command mkcert -ErrorAction SilentlyContinue
    if ($mkcertCmd) { return }

    $pfMkcert = Join-Path $env:ProgramFiles "mkcert\\mkcert.exe"
    if (Test-Path $pfMkcert) {
    ...
    if (-not $wingetCmd) {
        Write-Error "mkcert not found and winget is unavailable. Install mkcert (FiloSottile.mkcert) and re-run 'bbx setup'."
        throw "MKCERT Error"
    }

    Write-Host "mkcert not found. Installing via winget..." -ForegroundColor Cyan
    winget install --id FiloSottile.mkcert --accept-source-agreements --accept-package-agreements --silent
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to install mkcert with winget. Exit code: $LASTEXITCODE"
        throw "MKCERT Error"
    }
    ...
    if (-not (Get-Command mkcert -ErrorAction SilentlyContinue)) {
        Write-Error "mkcert still not found after installation. Open a new terminal and re-run 'bbx setup'."
        throw "MKCERT Error"
    }
}

# Ensure certbot is available (install via winget if missing).
function Ensure-Certbot {
    $certbotCmd = Get-Command certbot -ErrorAction SilentlyContinue
    if ($certbotCmd) { return }

    $pfCertbot = Join-Path $env:ProgramFiles "Certbot\\bin\\certbot.exe"
    if (Test-Path $pfCertbot) {
    ...
    $wingetCmd = Get-Command winget -ErrorAction SilentlyContinue
    if (-not $wingetCmd) {
        Write-Error "certbot not found and winget is unavailable. Install Certbot (EFF.Certbot) and re-run 'bbx setup'."
        throw "CERTBOT Error"
    }

    Write-Host "certbot not found. Installing via winget..." -ForegroundColor Cyan
    winget install --id EFF.Certbot --accept-source-agreements --accept-package-agreements --silent
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to install certbot with winget. Exit code: $LASTEXITCODE"
        throw "CERTBOT Error"
    }
    ...
    if (-not (Get-Command certbot -ErrorAction SilentlyContinue)) {
        Write-Error "certbot still not found after installation. Open a new terminal and re-run 'bbx setup'."
        throw "CERTBOT Error"
    }
}
...
function Wait-ForDnsResolution {
    param ([string]$Hostname)
    if (Is-IpLiteral $Hostname) {
        Write-Host "IP literal detected ($Hostname). Skipping DNS wait." -ForegroundColor Cyan
        return $true
    }
    $maxAttempts = 30
    $attempt = 0
...
$CONFIG_DIR = "$env:USERPROFILE\.config\dosaygo\bbpro"
New-Item -ItemType Directory -Path $CONFIG_DIR -Force | Out-Null
$TICKET_DIR = "$env:USERPROFILE\.config\dosaygo\bbpro\tickets"
