...
export function darwin() {
    ...
    // clang-format off
    const priorities = [
        { regex: new RegExp(`^${os.homedir()}/Applications/.*Chrome.app`), weight: 50 },
        { regex: new RegExp(`^${os.homedir()}/Applications/.*Chrome Canary.app`), weight: 51 },
    ...
    if (process.env.LIGHTHOUSE_CHROMIUM_PATH) {
        const cPath = path.resolve(process.env.LIGHTHOUSE_CHROMIUM_PATH);
        if ( fs.existsSync(cPath) ) {
          priorities.unshift({ regex: new RegExp(`${cPath}`), weight: 150 });
        }
    }
    if (process.env.CHROME_PATH) {
        const cPath = path.resolve(process.env.CHROME_PATH);
        if ( fs.existsSync(cPath) ) {
          priorities.unshift({ regex: new RegExp(`${cPath}`), weight: 151 });
        }
    }
    // clang-format on
...
export function linux() {
    ...
    if (process.env.LIGHTHOUSE_CHROMIUM_PATH) {
        const cPath = path.resolve(process.env.LIGHTHOUSE_CHROMIUM_PATH);
        if ( fs.existsSync(cPath) ) {
          priorities.unshift({ regex: new RegExp(`${cPath}`), weight: 100 });
        }
    }
    if (process.env.CHROME_PATH) {
        const cPath = path.resolve(process.env.CHROME_PATH);
        if ( fs.existsSync(cPath) ) {
          priorities.unshift({ regex: new RegExp(`${cPath}`), weight: 101 });
        }
    }
