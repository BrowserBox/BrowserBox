  ...
  async function start() {
    ...
    try {
      Object.assign(SSL_OPTS,{
        key: fs.readFileSync(Path.join(os.homedir(), CONFIG.sslcerts(process.env.DOCS_PORT), 'privkey.pem')),
        cert: fs.readFileSync(Path.join(os.homedir(), CONFIG.sslcerts(process.env.DOCS_PORT), 'fullchain.pem')),
      });
      secure = true;
    } catch(e) {
